---
title: "courtsports_intent_testing"
format: docx
editor: visual
---

```{r}
wbb_fp <- read_csv("wbb_fp.csv")
mbb_fp <- read_csv("mbb_fp.csv")
```
...
```{r}
# Summary statistics BEFORE exclusion WBB
summary_before <- wbb_fp %>%
  summarise(
    Total_Entries = n(),
...
```{r}
#assumption that more negative is slower 
#-1.2 m/s is "slower" than -2.0 m/s but "faster" than -0.8 m/s.
library(ggplot2)
library(dplyr)
...
comparison_df <- data.frame(
  ...
  Before_Exclusion = c(-1.03234	, -1.0035, -1.6192, -0.5513	, 0.180953
),
  After_Exclusion = c(-0.9746204, -0.951, -1.199, -0.5513, 0.1213237)
)
...
ggplot(comparison_long, aes(x = Statistic, y = Value, fill = Condition)) +
  ...
  labs(title = "Comparison of Braking Velocity Statistics Before and After Exclusion (-1.2 m/s) WBB",
       x = "Statistic",
       y = "Value") +
  theme_minimal() +
...
```{r}
NEED to quantify this/validate 
